...
export default function Contact() {
    ...
    const [overlay, setOverlay] = useState(" hidden fixed overflow-hidden inset-0 h-screen w-full flex justify-center items-start md:items-center pt-10 md:pt-0 ")
    const [openModal, setOpenModal] = useState(" opacity-0 scale-150 -translate-y-full z-[2000] transform relative mx-5 max-w-[640px] w-full bg-secondary05 rounded-[16px] shadow-lg transition-opacity transition-transform duration-300")
    ...
    function clickHandler(value: boolean) {
        ...
        if (value) {
            ...
            setTimeout(() => {
                setOpenModal(openModal?.replace("opacity-0 scale-150 -translate-y-full ", ""))
            }, 100);
        } else {
            ...
            setTimeout(() =>
                ...
                setOverlay(overlay?.replace("flex", "hidden"))
                , 300);
        }
            ...
            <div className=' relative h-full ' >
                ...
                <div className=' w-full mt-8 py-10 flex flex-col gap-12 ' >
                    {contactinfo?.map((item, index) => (
                        <div key={index} className=' w-full px-8 border-b border-secondary04 pb-12 flex lg:flex-row flex-col lg:gap-0 gap-5 justify-between ' >
                            <div className=' max-w-[358px] flex flex-col gap-5 ' >
                                ...
                                <div className=' flex gap-4 items-center ' >
                                    ...
                                    <p role='buttone' onClick={() => clickHandler(true)} className=' underline leading-6 text-sm ' >Send Message</p>
                                </div>
                            </div>
                        ...
                        <div className='  ' >
                            ...
                            <form className=' w-full flex flex-col overflow-y-auto max-h-[450px] gap-6 pt-2 px-5 mb-[80px] pb-5 flex-1 ' >
                                ...
                                <div className=' w-full grid grid-cols-1 lg:grid-cols-2 gap-6 ' >
                                    ...
                                    <CustomInput placeholder='Your Last Name' label='Last Name (required)' width='100%' />
